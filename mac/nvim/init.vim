...
let g:python3_host_prog = '~/.pyenv/bin/python3'
let g:python2_host_prog = '~/.pyenv/bin/python'
...
Plug 'justinmk/vim-sneak'                                                             "光标快速跳转插件
Plug 'lervag/vimtex'                                                                  "LaTeX编写辅助插件
...
function Test3()
	let list = ["哈哈哈", "呃呃呃"]
	call fzf#run({'source': list, 'window':{'width': 0.5, 'height': 0.5}, 'sink': function('ExecAction')})
endfunction
...
function ShowActionList()
	let action_list = [
		...
		\'cmd_F3: 相对行号显示切换', 
		\'cmd_F4: 取消文本高亮', 
		\'fun_UpdateFolderNames: 目录文件重命名',
		\'fun_LoadFolderNames: 读入目录文件', 
		\'fun_SortAndUniq: 排序并且去重', 
		\'fun_Test3: 再次选择', 
	\]
	call fzf#run({'source': reverse(action_list), 'window':{'width': 0.5, 'height': 0.5}, 'sink': function('ExecAction')})
endfunction

function ExecAction(line)
	let arr1 = split(a:line, ":")
	let arr2 = split(arr1[0], "_")
	if arr2[0] == "cmd"
		call feedkeys(':'.arr2[1]."\<CR>", 'n')
	elseif arr2[0] == "fun"
		call feedkeys(':call '.arr2[1].'()'."\<CR>", 'n')
	endif
endfunction
